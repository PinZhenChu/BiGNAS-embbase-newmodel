...
# 原本的寫法：
        ...
        # ✅ 改成這樣 (標準的 NeuMF 預測頭結構)：
        self.source_preds = nn.Sequential(
            nn.Linear(self.pred_input_dim, self.hidden_dim),
            nn.LeakyReLU(),
            nn.Linear(self.hidden_dim, 1)
        )
        self.target_preds = nn.Sequential(
            nn.Linear(self.pred_input_dim, self.hidden_dim),
            nn.LeakyReLU(),
            nn.Linear(self.hidden_dim, 1)
        )
        self.replace_target_preds = nn.Sequential(
            nn.Linear(self.pred_input_dim, self.hidden_dim),
            nn.LeakyReLU(),
            nn.Linear(self.hidden_dim, 1)
        )
    ...
    def forward(self, source_edge_index, target_edge_index, link, is_source=True):
        ...
        # out = x.sigmoid()
        # return out
        # 在 model.py 中修改 forward 函數的最後兩行：
# ✅ 改成這樣：
    ...
    def init_parameters(self):
        # 1. 替換掉原本的 normal_，改用 Xavier Uniform 讓初始向量具備足夠的變異數
        nn.init.xavier_uniform_(self.embedding.weight)
        
        # 2. 針對模型中所有的線性層 (包含 user_mix_linear, source_preds, target_preds) 進行初始化
